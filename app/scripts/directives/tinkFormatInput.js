  ...
  .directive('tinkFormatInput', ['dateCalculator','$window','safeApply','$compile',function (dateCalculator,$window,safeApply,$compile) {
    return {
      ...
      compile:function(template,attr){
          ...
          //template.html(angular.element('<input ng-model="ngModel" type="date"/>'));
          //template.find('input').removeAttr('tink-format-input').removeAttr('data-date').removeAttr('data-format');.removeAttr('data-placeholder');
          //template.find('input').prop('type','date');
           return {
           pre: function() { },
